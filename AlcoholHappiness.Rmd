...
```{r}
#calculate mean value of happiness on corresponding feature and save them as subset dataframe + realized there were more different features for country and happiness
y2015_2016 <-
  happinessCSV %>% 
  group_by(Country) %>%
...
So, based on the graph we created for the combined table, shows that alcohol consumption does have a positive correlation with happiness scores. When alcohol consumption in a country is high, the happiness score seems to be higher, when alcohol consumption is low in a country the happiness score seems to be low as well. However, the correlation seems to be less obvious as alcohol consumption goes over 12 alcohol level on smooth regression.
